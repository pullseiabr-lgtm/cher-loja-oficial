<template>
    ...
    <button type="button" @click="add" data-modal="#promotionSectionModal" class="db-btn h-[37px] text-white bg-primary">
        <i class="lab lab-line-add-circle"></i>
        <span>Adicionar Promoção</span>
    </button>

    <div id="promotionSectionModal" class="modal">
                ...
                <form @submit.prevent="save" class="d-block w-full">
                    <div class="form-row">
                        <div class="form-col-12">
                            <label for="promotion_id" class="db-field-title required">Promoção</label>
...
export default {
    name: "PromotionSectionPromotionCreateComponent",
    ...
    methods: {
        ...
        save: function () {
            try {
                this.loading.isActive = true;
                this.$store.dispatch("promotionSectionPromotion/save", this.props).then(() => {
                    appService.modalHide();
                    this.loading.isActive = false;
                    alertService.successFlip(0, "Promoção");
